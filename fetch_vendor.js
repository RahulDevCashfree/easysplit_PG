...
// Construct Vendor Details URL
const getVendorURL = `${baseURL}/easy-split/vendors/${vendorId}`;

// Function to Fetch Vendor Details
const fetchVendorDetails = async () => {
  try {
    console.log(`ℹ️ Fetching details for Vendor ID: ${vendorId}`);

    const response = await axios.get(getVendorURL, {
      headers: {
    ...
    console.log("✅ Vendor Details Fetched Successfully:", response.data);

    // Save Response to File
    const responseFilePath = path.join(__dirname, "vendor_details_response.json");
